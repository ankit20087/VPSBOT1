...
ADMIN_IDS = [930577300]  # Changed from string to list of integers
BOT_TOKEN = "7705589733:AAE-QQirF6vBKl3g5SLs7rOBzXzvys_5Gxs"
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id):
    ...
    try:
        ...
        output = stdout.decode().replace("", "@ANKITVIPDDOS")  # Replace with your name
        error = stderr.decode()
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @ANKITVIPDDOS"  
...
def send_welcome(message):
    ...
    welcome_message = (f"Welcome, {username}! To @ANKITVIPDDOS\n\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*You are not registered. Please redeem A key To Owner:- @ANKITVIPDDOS*", parse_mode='Markdown')
        return

    # Check if the user's key is still valid
    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*Your key has expired. Please redeem A key To Owner:- @ANKITVIPDDOS.*", parse_mode='Markdown')
        return
...
def my_account(message):
    ...
    if found_user:
        ...
        account_info = "Please redeem A key To Owner:- @ANKITVIPDDOS."
